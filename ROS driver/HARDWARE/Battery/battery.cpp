...
void Battery::init()
{
	...
	RCC_APB2PeriphClockCmd(RCC_APB2Periph_ADC1 | BATTERY_GPIO_CLK, ENABLE);
	RCC_AHBPeriphClockCmd(RCC_AHBPeriph_DMA1, ENABLE);

	GPIO_InitStructure.GPIO_Pin     = BATTERY_PIN;
	GPIO_InitStructure.GPIO_Mode    = GPIO_Mode_AIN;
	GPIO_Init(BATTERY_GPIO_PORT, &GPIO_InitStructure);
...
void Battery::filter()
{
	static u16 a[filter_count] = { 0 };
	static int b = 0;
	u32 c = 0;
	
	if(b < filter_count)
	{
		a[b] = ADC_ConvertedValue[0];
		b++;
		return;
	}
	for(b = 0; b < filter_count; b++)
	{
		c+=a[b];
	}
	c /= filter_count;
	volt_filter = (c * 3.33 * 11.1) / 4095.0;;
	b = 0;
}
